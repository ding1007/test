...
def compute_statistics_jit(overlaps,
    ...
    for i in range(gt_size):
        ...
        for j in range(det_size):
            if (ignored_det[j] == -1): #其他类别
                continue
            if (assigned_detection[j]):#已经被用过的框
                continue
            if (ignored_threshold[j]):#低于阈值的框
                continue
            ...
            if (not compute_fp and (overlap > min_overlap)
                ...
                det_idx = j
                valid_detection = dt_score # 遍历得到最高置信度得分
        ...
        if (valid_detection == NO_DETECTION) :
            fn += 1 #如果置信度没有变化 说明正样本未被检测到
        elif valid_detection != NO_DETECTION:
            tp += 1
            # thresholds.append(dt_scores[det_idx])
            thresholds[thresh_idx] = dt_scores[det_idx] #预测框的置信度
            thresh_idx += 1
            if compute_aos:
    ...
    if compute_fp:
        for i in range(det_size):
            if (not (assigned_detection[i] or ignored_det[i] == -1
                      or ignored_threshold[i])): #既不是其他类别、检测结果被标记为未使用、也不是低于阈值框的类别
                fp += 1
        nstuff = 0
        if metric == 0:
...
def calculate_iou_partly(dt_annos, gt_annos, metric, num_parts=50):
    ...
    Args:
        dt_annos (dict): Must from get_label_annos() in TJ4D_common.py.
        gt_annos (dict): Must from get_label_annos() in TJ4D_common.py.
...
def _prepare_data(gt_annos, dt_annos, current_class): #按类别
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
...
def eval_class(gt_annos,
               ...
               compute_aos=False,
               num_parts=200):
    """TJ4D eval. support 2d/bev/3d/aos eval. support 0.5:0.05:0.95 coco AP.
    这个函数是针对 TJ4D 数据集的评估函数，支持2D、BEV、3D、朝向评估，
    并且支持计算 0.5 到 0.95 之间以 0.05 为间隔的 COCO AP（Average Precision）。
    Args:
        gt_annos (dict): Must from get_label_annos() in TJ4D_common.py.
        dt_annos (dict): Must from get_label_annos() in TJ4D_common.py.
        ...
        num_parts (int): A parameter for fast calculate algorithm
        例如，假设有一个大型数据集，某个算法需要对其进行处理。为了提高处理速度，可以将数据集划分为多个部分，
        并利用多个处理单元同时处理这些部分。num_parts 就可以用来指定划分的部分数量，以便于并行处理或其他优化操作
    ...
    for m, current_class in enumerate(current_classes):
        ...
        # 此处[:, metric, m]  [:,是代表了什么维度
        for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
            thresholdss = []
            for i in range(len(gt_annos)): #按帧数
            ...
            for j, num_part in enumerate(split_parts):
                ...
                ignored_dets_part = np.concatenate( #会存在为空的情况
            ...
            for i in range(len(thresholds)):
                ...
                precision[m, k, i] = pr[i, 0] / (
                    pr[i, 0] + pr[i, 1])
                if compute_aos:
                    aos[m, k, i] = pr[i, 3] / (pr[i, 0] + pr[i, 1])
            for i in range(len(thresholds)):
...
def do_eval(gt_annos,
    ...
    if 'bev' in eval_types:
        ret = eval_class(gt_annos, dt_annos, current_classes, 1,
                         min_overlaps)
    ...
    mAP40_3d = None
    if '3d' in eval_types:
        ret = eval_class(gt_annos, dt_annos, current_classes, 2,
                         min_overlaps)
...
def TJ4D_eval(gt_annos,
    ...
    overlap_0_5 = np.array([[0.5, 0.25, 0.25, 0.5],
                            [0.5, 0.25, 0.25, 0.5],
                            [0.5, 0.25, 0.25, 0.5]])
    min_overlaps =overlap_0_5[None,:]  # [1, 3, 4]
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
        2: 'Cyclist',
    ...
    if not isinstance(current_classes, (list, tuple)): #这段代码检查 current_classes 是否是列表或元组类型，如果不是，则将其转换为包含单个元素的列表。这个检查是为了确保即使 current_classes 只包含单个类别，
        current_classes = [current_classes]            # 它也会被处理为列表的形式，以便在后续代码中能够统一处理列表类型的数据。
    current_classes_int = []
    # 这段代码的目的是将 current_classes 中的字符串类别转换为整数类别，以便在后续处理中可以统一地使用整数类别进行操作。
    ...
    min_overlaps = min_overlaps[:, :, current_classes] #根据class序号索引对应的Iou
    result = ''
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if mAP11_bbox is not None:
                result += 'bbox AP11:{:.4f}:\n'.format(
                    mAP11_bbox[j, i]) #str.format() 中 * 后的参数必须是可迭代的 删去了*号
            ...
            if i == 0:
                ...
                postfix = f'loose'
            prefix = f'TJ4D/{curcls_name}'
            if mAP11_3d is not None:
                ret_dict[f'{prefix}_3D_AP11'] =\
                    mAP11_3d[j, i]
            if mAP11_bev is not None:
                ret_dict[f'{prefix}_BEV_AP11'] =\
                    mAP11_bev[j, i]
            if mAP11_bbox is not None:
                ret_dict[f'{prefix}_2D_AP11'] =\
                    mAP11_bbox[j, i]
    ...
    if len(current_classes) > 1:
        ...
        if mAP11_3d is not None:
            ret_dict[f'TJ4D/Overall_3D_AP11'] = mAP11_3d[0]
        if mAP11_bev is not None:
            ret_dict[f'TJ4D/Overall_BEV_AP11'] =\
                mAP11_bev[0]
        if mAP11_bbox is not None:
            ret_dict[f'TJ4D/Overall_2D_AP11'] =\
                mAP11_bbox[0]
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            prefix = f'TJ4D/{curcls_name}'
            if mAP40_3d is not None:
                ret_dict[f'{prefix}_3D_AP40_{prefix}'] =\
                    mAP40_3d[j, i]
            if mAP40_bev is not None:
                ret_dict[f'{prefix}_BEV_AP40_{prefix}'] =\
                    mAP40_bev[j, i]
            if mAP40_bbox is not None:
                ret_dict[f'{prefix}_2D_AP40_{prefix}'] =\
                    mAP40_bbox[j, i]
    ...
    if len(current_classes) > 1:
        ...
        if mAP40_3d is not None:
            ret_dict[f'TJ4D/Overall_3D_AP40'] = mAP40_3d[0]
        if mAP40_bev is not None:
            ret_dict[f'TJ4D/Overall_BEV_AP40'] =\
                mAP40_bev[0]
        if mAP40_bbox is not None:
            ret_dict[f'TJ4D/Overall_2D_AP40'] =\
                mAP40_bbox[0]

    return result, ret_dict


def TJ4D_eval_coco_style(gt_annos, dt_annos, current_classes):
    """coco style evaluation of TJ4D.
