...
from mmdet3d.evaluation import TJ4D_eval
...
class TJ4DMetric(BaseMetric):
    ...
    Args:
        ...
        default_cam_key (str): The default camera for lidar to camera
            conversion. By default, TJ4D: 'CAM2', Waymo: 'CAM_FRONT'.
            Defaults to 'CAM2'.
    ...
    def __init__(self,
        ...
        self.default_prefix = 'TJ4D metric'
    ...
    def convert_annos_to_TJ4D_annos(self, data_infos: dict) -> List[dict]:
        """Convert loading annotations to TJ4D annotations.
        ...
        Returns:
            List[dict]: List of TJ4D annotations.
        """
            ...
            for i, annos in enumerate(data_annos):
                if len(annos['instances']) == 0:
                    ...
                    for instance in annos['instances']:
                        ...
                        TJ4D_annos['name'].append(label2cat[label])
                        TJ4D_annos['truncated'].append(instance['truncated'])
                        TJ4D_annos['occluded'].append(instance['occluded'])
                        TJ4D_annos['alpha'].append(instance['alpha'])
                        TJ4D_annos['bbox'].append(instance['bbox'])
                        TJ4D_annos['location'].append(instance['bbox_3d'][:3])
                        TJ4D_annos['dimensions'].append(
                            instance['bbox_3d'][3:6])
                        TJ4D_annos['rotation_y'].append(
                            instance['bbox_3d'][6])
                        TJ4D_annos['score'].append(instance['score'])
                    for name in TJ4D_annos:
                        TJ4D_annos[name] = np.array(TJ4D_annos[name])
                data_annos[i]['TJ4D_annos'] = TJ4D_annos
        return data_annos
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        参数：
            results (List[dict])：整个数据集的处理结果列表。
        返回值：
            Dict[str, float]：计算得到的指标。键是指标的名称，值是对应的结果。
        """
        ...
        self.data_infos = self.convert_annos_to_TJ4D_annos(pkl_infos)
        ...
        gt_annos = [
            self.data_infos[result['sample_idx']]['TJ4D_annos']
            for result in results
        ]

        for metric in self.metrics:
            ap_dict = self.TJ4D_evaluate(
                result_dict,
                gt_annos,
                metric=metric,
                logger=logger,
    ...
    def TJ4D_evaluate(self,
        ...
        for name in results_dict:
            ...
            ap_result_str, ap_dict_ = TJ4D_eval(
    ...
    def format_results(
        ...
        for name in results[0]:
            ...
            if 'pred_instances' in name and '3d' in name and name[
                ...
                result_list_ = self.bbox2result_TJ4D(net_outputs,
                ...
                result_dict[name] = result_list_
                    ...
                    name]:
                net_outputs = [result[name] for result in results]
                result_list_ = self.bbox2result_TJ4D2d(
                    net_outputs, sample_idx_list, classes, pklfile_prefix_,
                    submission_prefix_)
                result_dict[name] = result_list_
        return result_dict, tmp_dir

    def bbox2result_TJ4D(
            self,
        ...
        """Convert 3D detection results to TJ4D format for evaluation and test
        submission.
        ...
        Returns:
            List[dict]: A list of dictionaries with the TJ4D format.
        """
        ...
        det_annos = []
        print('\nConverting 3D prediction to TJ4D format')
    ...
    def bbox2result_TJ4D2d(
            ...
            pklfile_prefix: Optional[str] = None,
            submission_prefix: Optional[str] = None) -> List[dict]:
        """Convert 2D detection results to TJ4D format for evaluation and test
        submission.
        ...
        assert len(net_outputs) == len(self.data_infos), \
            'invalid list length of network outputs'
        det_annos = []
        print('\nConverting 2D prediction to TJ4D format')
        ...
        if submission_prefix is not None:
            ...
            mmengine.mkdir_or_exist(submission_prefix)
            print(f'Saving TJ4D submission to {submission_prefix}')
    ...
    def convert_valid_bboxes(self, box_dict: dict, info: dict) -> dict:
        ...
        Returns:
            ...
            这个函数执行了从三维边界框到二维边界框以及相机和 LiDAR 坐标系下的边界框的转换，
            并返回了转换后的有效预测框和相关信息。
        """
