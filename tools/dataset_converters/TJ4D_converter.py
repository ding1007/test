...
from .TJ4D_data_utils import  get_TJ4D_image_info
from .nuscenes_converter import post_process_coords
#TODO:
TJ4D_categories = ('Car','Pedestrian', 'Cyclist','Truck')


def convert_to_TJ4D_info_version2(info):
...
class _NumPointsInGTCalculater:
    ...
    def calculate_single(self, info):
        ...
        annos = info['annos']
        num_obj = len([n for n in annos['name'] if n != 'Other'])
...
def create_TJ4D_info_file(data_path,
                           pkl_prefix='TJ4D',
                           with_plane=False,
                           save_path=None,
                           relative_path=True):
    """Create info file of TJ4D dataset.
    ...
    TJ4D_infos_train = get_TJ4D_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, TJ4D_infos_train, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'TJ4D info train file is saved to {filename}')
    mmengine.dump(TJ4D_infos_train, filename)
    TJ4D_infos_val = get_TJ4D_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, TJ4D_infos_val, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'TJ4D info val file is saved to {filename}')
    mmengine.dump(TJ4D_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'TJ4D info trainval file is saved to {filename}')
    mmengine.dump(TJ4D_infos_train + TJ4D_infos_val, filename)
...
def _create_reduced_point_cloud(data_path,
    ...
    Args:
        ...
        front_camera_id (int, optional): The referenced/front camera ID.
            Default: 2.
    """
    TJ4D_infos = mmengine.load(info_path)

    for info in mmengine.track_iter_progress(TJ4D_infos):
        pc_info = info['point_cloud']
        image_info = info['image']
        calib = info['calib']
...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    cat2Ids = [
        dict(id=TJ4D_categories.index(cat_name), name=cat_name)
        for cat_name in TJ4D_categories
    ]
    coco_ann_id = 0
...
def generate_record(ann_rec, x1, y1, x2, y2, sample_data_token, filename):
    ...
    if repro_rec['category_name'] not in TJ4D_categories:
        return None
    cat_name = repro_rec['category_name']
    coco_rec['category_name'] = cat_name
    coco_rec['category_id'] = TJ4D_categories.index(cat_name)
